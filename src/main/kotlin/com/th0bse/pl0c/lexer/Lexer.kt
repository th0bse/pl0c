...
class Lexer {
  ...
  /** Iterate over the token stream, find expressions and convert them to postfix notation */
  ...
  private fun ArrayList<Token>.infixToPostfix() {
    ...
    for (token in this) {
      if (token is Num || token is Identifier) {
        ...
        when (token) {
          Symbol.LPAREN -> operatorStack.add(token)
          Symbol.RPAREN -> {
